...
from model.Net import Net as Net

parser = argparse.ArgumentParser(description="PyTorch BasicIRSTD train")
parser.add_argument("--model_names", default=['Net'], type=list, help="'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'RISTDnet'")
parser.add_argument("--dataset_names", default=['SIRST3'], type=list)
# SIRST3： NUAA RealScene-ISTD IRSTD-1K
...
parser.add_argument("--log_dir", type=str, default="./otherlogs/Net", help='path of log files')
...
config_vit = config.get_Net_config()
...
def train():
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        for idx_iter, (img, gt_mask) in enumerate(train_loader):
            ...
            noise_gaussian = torch.randn_like(img) * sigma
            noisy_img = img + noise_gaussian
            noise_thermal = img * torch.randn_like(img) * thermal_noise_factor
            noisy_img = noisy_img + noise_thermal
            preds_noise = net.forward(noisy_img)
            loss_mse = nn.MSELoss()(preds_noise[-1], preds[-1])
...
class Net(nn.Module):
    def __init__(self, model_name, mode):
        ...
        if model_name == 'Net':
            if mode == 'train':
                self.model = Net(config_vit, mode='train', deepsuper=True)
            else:
                self.model = Net(config_vit, mode='test', deepsuper=True)
    def forward(self, img):
        return self.model(img)

    def MSE_loss(self, preds,  preds_noise):
        if isinstance(preds, list):
            loss_total = 0
            for i in range(len(preds)):
                pred = preds[i]
                pred_noise = preds_noise[i]
                loss = nn.MSELoss()(pred, pred_noise)
